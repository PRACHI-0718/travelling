# 🌏 Explore Sydney Website

This is a simple multi-page website about Sydney, Australia.  
It showcases top tourist attractions, food, and culture of Sydney.  
Built using **HTML** and **CSS** and hosted on **GitHub Pages**.

---

## 🗂 Pages

- **Home** – Welcome page with hero image and introduction to Sydney.  
- **Places to Visit** – Top attractions in Sydney with cards linking to detailed pages.  
- **Opera House** – Detailed page about Sydney Opera House.  
- **Harbour Bridge** – Detailed page about Sydney Harbour Bridge.  
- **Bondi Beach** – Detailed page about Bondi Beach.  
- **Food & Culture** – Information about Sydney's vegetarian food and culture.

---

## 🌐 Live Website

[Visit the website](https://prachi-0718.github.io/travelling/)

---

## ⚡ How to Use

- Click the links in the **navigation bar** to explore different pages.  
- Click on any attraction card in the “Places to Visit” page to see detailed information.  
- All pages are linked internally and fully functional.

---

## ✨ Features

- Fully responsive layout  
- Multi-page navigation  
- CSS styling for cards, buttons, and sections  
- Hosted for free using GitHub Pages

---

## 🛠 Tech Used

- HTML  
- CSS  
- GitHub Pages
